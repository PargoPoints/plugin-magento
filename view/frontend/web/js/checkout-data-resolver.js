...
define([
  ...
  return {
    ...
    applyShippingAddress: function (isEstimatedAddress) {
      ...
      if (!shippingAddress) {
        isShippingAddressInitialized = addressList.some(function (
          addressFromList
        ) {
          if (
            checkoutData.getSelectedShippingAddress() ==
            addressFromList.getKey()
          ) {
            //eslint-disable-line
    ...
    resolveShippingRates: function (ratesData) {
      ...
      if (quote.shippingMethod()) {
        availableRate = _.find(ratesData, function (rate) {
          return (
            rate["carrier_code"] == quote.shippingMethod()["carrier_code"] && //eslint-disable-line
    ...
    resolveBillingAddress: function () {
      ...
      if (selectedBillingAddress) {
        if (
          selectedBillingAddress == "new-customer-address" &&
          newCustomerBillingAddressData
        ) {
          //eslint-disable-line
          selectBillingAddress(
            createBillingAddress(newCustomerBillingAddressData)
          );
        } else {
